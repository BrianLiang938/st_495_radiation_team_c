...
dtm <- TermDocumentMatrix(descrip, control=list(weighting = function(x) weightTfIdf(x, normalize=TRUE), wordLengths = c(2,Inf)))
...
#lsaSpace$dk
attribute_id <- as.factor(post_added$ATTRIBUTE_ID)
lsa_df <- data.frame(attribute_id, lsaSpace$dk)
#lsa_df

err <- rep(NA, 500)
for(i in 1:500) {
  train_index <- sample(nrow(lsa_df), .7*nrow(lsa_df))
...
train_index <- sample(nrow(mod_lsa_df), .7*nrow(mod_lsa_df))
rf_model = randomForest(attribute_id~., data=lsa_df, train=train_index, mtry=8, importance=TRUE)
predictions = predict(rf_model, newdata=lsa_df[-train_index,])
mean(predictions!=lsa_df[-train_index,]$attribute_id)
```